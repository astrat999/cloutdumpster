...
const vapidPublicKey =
	'BFhep7uswxcrFtbWlCx6-dQE9VZjFIeAHWg4EySQf-jQk26Ka0PXuSamOVLvEWnQwq-twUNrKdg01AOQ5pEmBNc'; // The same public key from your .env file
const vapidPrivateKey = functions.config().vapid?.private_key;

if (vapidPrivateKey) {
	webpush.setVapidDetails('mailto:astrat999@gmail.com', vapidPublicKey, vapidPrivateKey);
}
...
export const decayCloutScores = onSchedule({ schedule: 'every 24 hours' }, async (event) => {
	...
	const inactiveUsersQuery = db.collection('users').where('lastActive', '<', inactivityThreshold);
...
export const sendWhisperNotification = onDocumentWritten(
	'users/{userId}/whispers/{whisperId}',
	async (event) => {
...
export const stripeWebhook = onRequest(async (request, response) => {
	...
	if (event.type === 'checkout.session.completed') {
		...
		if (coinsToGrant && userId) {
			...
			logger.error(
				`Invalid purchase data: userId=${userId}, priceId=${priceId}, coins=${coinsToGrant}`
			);
		}
	}
...
export const migrateRoastsToWhispers = onCall({ region: 'us-central1' }, async (request) => {
