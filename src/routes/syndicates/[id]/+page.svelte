<script lang="ts">
    ...
    $: isUserSyndicateLeader = $user && data.userSyndicate && data.userSyndicate.leaderId === $user.uid;
    ...
    async function launchSmearCampaign() {
        ...
        if (!data.userSyndicate || data.userSyndicate.treasury < SMEAR_COST) {
            ...
            <div class="space-y-8">
                ...
                {#if isRivalSyndicate && isUserSyndicateLeader}
                    <div class="flex justify-center space-x-4 mb-8">
                        <button 
                            class="px-6 py-3 rounded-xl font-semibold transition-colors {activeTab === 'overview' ? 'bg-royal text-white' : 'bg-silk/30 text-gray-300 hover:text-white'}"
                            on:click={() => activeTab = 'overview'}
                        >
                            📊 Overview
                        </button>
                        <button 
                            class="px-6 py-3 rounded-xl font-semibold transition-colors {activeTab === 'actions' ? 'bg-red-600 text-white' : 'bg-silk/30 text-gray-300 hover:text-white'}"
                ...
                {:else if activeTab === 'actions'}
                    <!-- === WARFARE ACTIONS TAB === -->
                    <div class="bg-silk/30 rounded-2xl border border-red-500/20 p-8">
                        <div class="text-center mb-8">
                            <div class="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-red-600 to-red-800 rounded-full flex items-center justify-center">
                                <span class="text-4xl">⚔️</span>
                            </div>
                            
                            <h2 class="font-display text-3xl font-bold text-white mb-4">Syndicate Warfare</h2>
                            <p class="text-gray-400 text-lg mb-8 max-w-2xl mx-auto">
                                Launch coordinated attacks against <span class="text-red-400 font-semibold">[{syndicate.tag}]</span>. 
                                All actions are funded by your Syndicate treasury and will be publicly logged.
                            </p>

                            <!-- User's Syndicate Info -->
                            {#if data.userSyndicate}
                                <div class="bg-velvet/50 rounded-xl p-4 mb-8 max-w-md mx-auto">
                                    <h3 class="text-white font-semibold mb-2">Your Treasury: [{data.userSyndicate.tag}]</h3>
                                    <div class="text-gold text-2xl font-bold">{data.userSyndicate.treasury.toLocaleString()} CloutCoin™</div>
                                </div>
                            {/if}
                        </div>
                        ...
                        <div class="grid md:grid-cols-2 gap-8">
                            <!-- Smear Campaign -->
                            <div class="bg-velvet/50 rounded-xl p-6 border border-yellow-500/20">
                                <div class="text-center mb-6">
                                    ...
                                    <h3 class="font-display text-xl font-bold text-white mb-2">Smear Campaign</h3>
                                    <p class="text-gray-400 text-sm mb-4">
                                        Launch coordinated character assassination against all rival members. 
                                        Adds permanent "Anonymous Rival" roasts to their profiles.
                                    </p>
                                    <div class="text-yellow-400 font-bold text-lg">Cost: 2,500 CloutCoin™</div>
                                </div>

                                <button 
                                    class="w-full bg-gradient-to-r from-yellow-500 to-orange-600 text-black font-bold py-3 rounded-xl hover:from-orange-600 hover:to-orange-700 transition-colors disabled:opacity-50 flex items-center justify-center space-x-2"
                                    on:click={launchSmearCampaign}
                                    disabled={isLaunchingSmear || !data.userSyndicate || data.userSyndicate.treasury < 2500}
                                >
                                    {#if isLaunchingSmear}
                            ...
                            <!-- Clout Heist -->
                            <div class="bg-velvet/50 rounded-xl p-6 border border-red-500/20">
                                <div class="text-center mb-6">
                                    <div class="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-red-500 to-red-700 rounded-full flex items-center justify-center">
                                        <span class="text-2xl">💎</span>
                                    </div>
                                    <h3 class="font-display text-xl font-bold text-white mb-2">Clout Heist</h3>
                                    <p class="text-gray-400 text-sm mb-4">
                                        Coordinate a direct assault on rival clout scores. Steals 1-5 points from each member, 
                                        redistributes 50% among your Syndicate.
                                    </p>
                                    <div class="text-red-400 font-bold text-lg">Cost: 5,000 CloutCoin™</div>
                                </div>

                                <button 
                                    class="w-full bg-gradient-to-r from-red-500 to-red-700 text-white font-bold py-3 rounded-xl hover:from-red-600 hover:to-red-800 transition-colors disabled:opacity-50 flex items-center justify-center space-x-2"
                                    on:click={launchCloutHeist}
                                    disabled={isLaunchingHeist || !data.userSyndicate || data.userSyndicate.treasury < 5000}
                                >
                                    {#if isLaunchingHeist}
                                        <div class="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                                    {:else}
                        ...
                        {#if warfareError}
                            <div class="mt-6 p-4 bg-red-500/20 border border-red-500/30 rounded-xl">
                                <p class="text-red-400 text-center font-semibold">{warfareError}</p>
                            </div>
                        {/if}

                        {#if warfareSuccess}
                            <div class="mt-6 p-4 bg-green-500/20 border border-green-500/30 rounded-xl">
                                <p class="text-green-400 text-center font-semibold">{warfareSuccess}</p>
                            </div>
                        {/if}

                        <!-- Warning -->
                        <div class="mt-8 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                            <p class="text-yellow-400 text-center text-sm">
                                ⚠️ All attacks are publicly logged and will trigger retaliation. 
                                Warfare escalates quickly. Proceed with caution.
                            </p>
                        </div>
                    </div>
                {/if}
